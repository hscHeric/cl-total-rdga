...
Population::Population(size_t size,
  ...
  if (chromosomes.size() != size) {
    std::string str = "Initial chromosome count of " +
                      std::to_string(chromosomes.size()) +
                      " must match population size of " + std::to_string(size);
...
void Population::evolve_l(const ListGraph &graph, size_t tournamentSize,
                          double crossoverRate, double elitismRate,
                          double mutationRate) {
  ...
  if (size_elitism > 0) {
    ...
    std::sort(temp_pop.begin(), temp_pop.end(),
...
void Population::evolve_m(const MatrixGraph &graph, size_t tournamentSize,
                          double crossoverRate, double elitismRate,
                          double mutationRate) {
...
void Population::mutation_l(Chromosome &chr, float mutationRate,
...
void Population::mutation_m(Chromosome &chr, float mutationRate,

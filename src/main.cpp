...
int main( int argc, char * argv[] ) {
...
#if DEBUG
      std::cout << "A execução foi interrompida pois o grafo contém vértices " "isolados." << std::endl;
#endif
      return 1;
    }
  ...
  auto thread_func = [&]( size_t trial_id ) {
    ...
    std::cout << "  Tempo (microssegundos): " << result.elapsed_micros << std::endl;
    std::cout << "  Corresponde a heurística: " << ( result.matches_heuristic ? "Sim" : "Não" );
  ...
  for ( size_t trial = 0; trial < params.trials; ++trial ) {
    #if DEBUG
    std::cout << "\n===== Executando trial " << ( trial + 1 ) << " de " << params.trials << " =====\n" << std::endl;
    #endif
    TrialResult result;
    if ( graph_is_matrix ) {
      result = run_genetic_algorithm_m( graph_m, params );
  ...
  #if DEBUG
  std::cout << "\nTodos os resultados foram salvos em: " << params.output_file << std::endl;
  return 0;
  #endif  // DEBUG
#endif    // !IRACE
}
...
AlgorithmParams parse_args( int argc, char * argv[] ) {
  ...
  params.file_path = argv[1];  // No modo normal, o caminho do arquivo está em argv[1]
#endif
...
void ensure_csv_header( const std::string & filename ) {
  ...
  if ( !file_exists ) {
    file.open( filename );
    file << "graph_name,graph_order,graph_size,fitness_value,elapsed_time(" "microsecond),matches_heuristic,heuristic_matched,is_valid," "density,is_dense\n";
  } else {
  ...
  for ( int v : graph.get_vertices() ) {
    ...
    for ( int i = 0; i < static_cast<int>( neighbors.size() ); ++i ) {
      if ( neighbors[i] == 1 ) {
        ...
        if ( ( label == 0 && neighbor_label == 2 ) || ( label > 0 && neighbor_label > 0 ) ) {
    ...
    if ( !hasValidNeighbor ) {
#if DEBUG
      std::cout << "Solução inválida: vértice " << v << " com f(v) = " << label << " não possui vizinho válido." << std::endl;
#endif
      return false;
    }
  }
...
void write_result_to_csv( const std::string & filename, const TrialResult & result ) {
  ...
  file << result.graph_name << "," << result.node_count << "," << result.edge_count << "," << result.fitness << "," << result.elapsed_micros << ","
       << ( result.matches_heuristic ? "yes" : "no" ) << "," << result.matched_heuristic << "," << ( result.is_valid ? "yes" : "no" )
...
TrialResult run_genetic_algorithm_m( const MatrixGraph & graph, const AlgorithmParams & params ) {
  ...
  size_t pop_size = static_cast<size_t>( graph.order() / params.population_factor );
  ...
  std::vector<std::pair<std::string, decltype( heuristic_h1 )>> heuristics = {
    { "h1", heuristic_h1 }, { "h2", heuristic_h2 }, { "h3", heuristic_h3 }, { "h4", heuristic_h4 }, { "h5", heuristic_h5 }
  };
  ...
  for ( const auto & [name, heuristic] : heuristics ) {
    ...
    if ( valid_totalrd_m( graph, heuristic ) ) {
      ...
      std::cout << "[ERRO - MatrixGraph] Solução utilizando " << name << " é INVÁLIDA\n";
    }
  ...
  while ( generation < params.generations && stagnant_generations < params.max_stagnant ) {
    population.evolve_m( graph, params.tournament_size, params.crossover_rate, params.elitism_rate, params.mutation_rate );

    const auto & chromosomes = population.getChromosomes();
    auto         best_it     = std::min_element(
      chromosomes.begin(), chromosomes.end(), []( const Chromosome & a, const Chromosome & b ) { return a.get_fitness() < b.get_fitness(); } );
...
#if DEBUG
      std::cout << "Nova melhor solução na geração " << generation << ": fitness = " << best_fitness << std::endl;
#endif
    } else {
...
#if DEBUG
  std::cout << "Algoritmo genético concluído após " << generation << " gerações" << std::endl;
  ...
  if ( !is_valid ) {
#if DEBUG
    std::cout << "ATENÇÃO: A melhor solução encontrada não é válida!" << std::endl;
#endif
  } else {
#if DEBUG
  ...
  double density = static_cast<double>( graph.size() ) / ( graph.order() * ( graph.order() - 1 ) / 2.0 );
...
TrialResult run_genetic_algorithm_l( const ListGraph & graph, const AlgorithmParams & params ) {
  ...
  for ( const auto & [name, heuristic] : heuristics ) {
    ...
    if ( valid_totalrd_l( graph, heuristic ) ) {
      ...
      std::cout << "[ERRO - ListGraph] Solução utilizando " << name << " é INVÁLIDA\n";
    }
  ...
  while ( generation < params.generations && stagnant_generations < params.max_stagnant ) {
    population.evolve_l( graph, params.tournament_size, params.crossover_rate, params.elitism_rate, params.mutation_rate );

  ...
  while ( H.order() > 0 ) {
    ...
    while ( !isolatedVertices.empty() ) {
      ...
      for ( size_t i = 0; i < zNeighbors.size(); ++i ) {
        if ( zNeighbors[i] && chromosome.get_value( static_cast<int>( i ) ) == LABEL_ONE ) {
          hasLabel1Neighbor = true;
          break;
        }
      }
...
Chromosome HeuristicGenerators::h3_l( const ListGraph & graph ) {
  ...
  while ( H.order() > 0 ) {
    ...
    std::vector<int> neighbors_copy = neighbors;  // Cópia para evitar modificar durante iteração
    for ( int neighbor : neighbors_copy ) {
      if ( H.contains( neighbor ) ) {
        H.remove_vertex( neighbor );
      }
...
Chromosome HeuristicGenerators::h4_l( const ListGraph & graph ) {
  ...
  while ( H.order() > 0 ) {
    ...
    std::sort( sortedNeighbors.begin(), sortedNeighbors.end(), [&H]( int a, int b ) { return H.degree( b ) < H.degree( a ); } );
...
Chromosome HeuristicGenerators::h4_m( const MatrixGraph & graph ) {
  ...
  while ( H.order() > 0 ) {
    ...
    std::sort(
      neighborDegrees.begin(), neighborDegrees.end(), []( const std::pair<int, int> & a, const std::pair<int, int> & b ) { return a.second > b.second; } );
